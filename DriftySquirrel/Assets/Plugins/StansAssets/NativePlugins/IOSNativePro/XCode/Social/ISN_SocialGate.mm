...
const char* FACEBOOK_LISTENER = "SA.IOSNative.Social.ISN_Facebook+NativeListener";
const char* TWITTER_LISTENER = "SA.IOSNative.Social.ISN_Twitter+NativeListener";
const char* INSTAGRAM_LISTENER = "SA.IOSNative.Social.ISN_Instagram+NativeListener";
const char* MAIL_LISTENER = "SA.IOSNative.Social.ISN_Mail+NativeListener";
const char* TEXT_MESSAGE_LISTENER = "SA.IOSNative.Social.ISN_TextMessage+NativeListener";
...
- (void) mediaShare:(NSString*)text imagesList:(NSArray<NSString *> *)imagesList;
...
extern "C" {
    ...
    void _ISN_TwPostGIF(char* text, char* url) {
        [ISN_Logger LogNativeMethodInvoke:"_ISN_TwPostGIF" data:ISN_ConvertToChar([NSString stringWithFormat:@"text: %s url: %s", text, url])];

        [[ISN_SocialGate sharedInstance] twitterPostGif:[NSString stringWithUTF8String: text] url:[NSString stringWithUTF8String: url]];
    }
    ...
    void _ISN_MediaShare(char* text, char* encodedMedia) {
        [ISN_Logger LogNativeMethodInvoke:"_ISN_MediaShare" data:ISN_ConvertToChar([NSString stringWithFormat:@"text: %s encodedMedia: %s", text, encodedMedia])];

        NSString *status = [NSString stringWithUTF8String: text];
        NSArray* imagesList = charToNSArray(encodedMedia);
        
        [[ISN_SocialGate sharedInstance] mediaShare:status imagesList:imagesList];
    }
