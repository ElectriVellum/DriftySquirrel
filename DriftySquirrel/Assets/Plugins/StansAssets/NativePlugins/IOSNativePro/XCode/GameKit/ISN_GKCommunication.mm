...
@implementation ISN_GKSavedGame
    -(id) init { return self = [super init]; }
    -(id) initWithGKSavedGame:(GKSavedGame *) savedGame {
        self = [super init];
        if(self) {
            ...
            if(savedGame.modificationDate != NULL) {
                ...
                NSInteger interval = [zone secondsFromGMTForDate:savedGame.modificationDate];
                mydate = [mydate dateByAddingTimeInterval:interval];
                self.m_modificationDate = [mydate timeIntervalSince1970];
            } else {
